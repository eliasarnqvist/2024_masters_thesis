...
// Elias: 4 new parameters, store qreg1 in qshort, qreg2 in baseline
...
void energy_init(json_t *json_config, void **user_config)
{
    ...
    if (!json_is_object(json_config)) {
        ...
        // Elias:
        config->reg1_start = json_integer_value(json_object_get(json_config, "reg1_start"));
        config->reg1_stop = json_integer_value(json_object_get(json_config, "reg1_stop"));
        config->reg2_start = json_integer_value(json_object_get(json_config, "reg2_start"));
        config->reg2_stop = json_integer_value(json_object_get(json_config, "reg2_stop"));
                     ...
                     void *user_config)
{
    ...
    const int64_t global_pregate = config->pregate;
    ...
    int64_t short_gate_start = local_trigger_position - config->pregate;
    int64_t long_gate_start = local_trigger_position - config->pregate;
    
    int64_t reg1_start_rel = local_trigger_position - config->pregate + config->reg1_start;
    int64_t reg2_start_rel = local_trigger_position - config->pregate + config->reg2_start;
    ...
    int64_t reg1_stop_rel = local_trigger_position - config->pregate + config->reg1_stop - 1;
    int64_t reg2_stop_rel = local_trigger_position - config->pregate + config->reg2_stop - 1;
    ...
    const double qreg1 = config->curve_integral[reg1_stop_rel]
                         - config->curve_integral[reg1_start_rel - 2];
    const double qreg2 = config->curve_integral[reg2_stop_rel]
                         - config->curve_integral[reg2_start_rel - 2];

    const double PSD = (qlong != 0) ? (qlong - qshort) / qlong : (config->PSD_min - 1);
    const double PSD_Elias = (qreg2 != 0) ? qreg1 / qreg2 : (config->PSD_min - 1);
    ...
    const double scaled_qreg1 = qreg1 * config->integrals_scaling
                                * (config->pulse_polarity == POLARITY_POSITIVE ? 1.0 : -1.0);
    const double scaled_qreg2 = qreg2 * config->integrals_scaling
                                * (config->pulse_polarity == POLARITY_POSITIVE ? 1.0 : -1.0);

    const uint64_t long_qshort = (uint64_t)round(scaled_qshort);
    const uint64_t long_qlong = (uint64_t)round(scaled_qlong);

    const uint64_t long_qreg1 = (uint64_t)round(scaled_qreg1);
    const uint64_t long_qreg2 = (uint64_t)round(scaled_qreg2);
    ...
    uint16_t int_qreg1 = long_qreg1 & UINT16_MAX;
    uint16_t int_qreg2 = long_qreg2 & UINT16_MAX;
    ...
    if (int_qreg1 > UINT16_MAX)
    {
        int_qreg1 = UINT16_MAX;
    }
    if (int_qreg2 > UINT16_MAX)
    {
        int_qreg2 = UINT16_MAX;
    }
    ...
    if (scaled_qlong < config->energy_threshold || PSD_Elias < config->PSD_min || PSD_Elias > config->PSD_max) {
        // Discard the event
        ...
        //(*events_buffer)[0].qshort = int_qreg1;
        (*events_buffer)[0].qlong = int_qlong;
        //(*events_buffer)[0].baseline = int_baseline;
        //(*events_buffer)[0].baseline = int_qreg2;
        (*events_buffer)[0].baseline = PSD_Elias * 1000;
